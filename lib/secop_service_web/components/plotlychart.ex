    ...
    socket =
      case path do
        {host, port, module, parameter} ->
          ...
          secop_obj =
            Map.put(secop_obj, :parameter_id, param_id) |> Map.put(:parameter_topic, parmam_topic)
  ...
  def update(%{value_update: value_update, pubsub_topic: pubsub_topic} = assigns, socket) do
    ...
    socket = case qualifiers do
        %{t: timestamp} ->
              ...
              update_data = %{
                x: [[timestamp]],  # Add one timestamp to the specified trace
                y: [[value]],      # Add one value to the specified trace
