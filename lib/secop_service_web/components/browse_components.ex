defmodule SecopServiceWeb.BrowseComponents do
        ...
        <div class="p-4 flex flex-wrap gap-2">
          <%= for {name, value} <- @ordered_members do %>
            <div class="flex items-center px-3 py-1.5 rounded-md border bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-200 border-gray-300 dark:border-gray-600">
              ...
              <span class="ml-2 px-2 py-0.5 rounded-full bg-white/75 dark:bg-gray-800/75 text-xs font-mono">{value}</span>
            </div>
          <% end %>
        </div>
      </div>
  ...
  defp get_color_class(code) do
    color_classes = %{
      :disabled => "bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-200 border-gray-300 dark:border-gray-600",
      :idle => "bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-100 border-green-200 dark:border-green-800",
      :warn => "bg-yellow-100 text-yellow-800 dark:bg-yellow-900 dark:text-yellow-100 border-yellow-200 dark:border-yellow-800",
      :busy => "bg-orange-100 text-orange-800 dark:bg-orange-900 dark:text-orange-100 border-orange-200 dark:border-orange-800",
  ...
  def status_tuple(assigns) do
    ...
    ordered_members = assigns.status_tuple["members"]
    |> Enum.at(0)
    |> Map.get("members")
      ...
      <:trigger class="text-left">
        ...
        <div class="border-4 border-zinc-300 dark:border-zinc-600 bg-white/50 dark:bg-gray-800/60 rounded-lg p-4">
          <ul class="text-sm text-gray-700 dark:text-gray-300 space-y-2">
            <.property prop_key="Description" key_class="text-gray-700 dark:text-gray-300 text-lg font-bold" value_class="text-lg font-semibold">
             {@module.description}
            </.property>
            ...
            <%= if group_name == nil do %>
              <div class="mt-4 border-l-4 border-transparent pl-4">
                <%= for parameter <- parameters do %>
                  <.parameter parameter={parameter} parameter_descr={@module_descr["accessibles"][parameter.name]}/>
                <% end %>
              </div>
            <% else %>
              ...
              <%= if group_name == nil do %>
                <div class="mt-4 border-l-4 border-transparent pl-4">
                  <%= for command <- commands do %>
                    <.command command={command} command_descr={@module_descr["accessibles"][command.name]} />
                  <% end %>
                </div>
              <% else %>
                <div class="mt-4 border-l-4 border-gray-500 pl-4">
  ...
  def command(assigns) do
    ...
    <div class="mb-4 bg-gray-300 dark:bg-gray-700 rounded-lg p-4 shadow-md">
        ...
        <!-- Parameter Name -->
        <div>
          <span class="text-xl font-bold text-gray-800 dark:text-white">{Util.display_name(@command.name)}:</span>
          <ul class="mt-2 text-sm text-gray-700 dark:text-gray-300">
            <!-- Description -->
