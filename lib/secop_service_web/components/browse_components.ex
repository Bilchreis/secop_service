defmodule SecopServiceWeb.BrowseComponents do
  ...
  def module(assigns) do
    ...
    <div class="bg-gray-700 rounded-lg p-4 mb-6 shadow-md">
      <!-- Module Name -->
      <div class="mb-4">
        <span class="text-2xl font-bold text-white">{Module.display_name(@module)}</span>
      </div>

      <!-- Module Properties -->
      <div class="border-4 border-zinc-500 bg-gray-800 rounded-lg p-4">
        <h3 class="text-lg font-bold text-purple-400 mb-2">Module Properties:</h3>
        <ul class="text-sm text-gray-300 space-y-2">
          <.property prop_key="Description" key_class="text-gray-300 text-sm font-bold">
            {@module.description}
          </.property>
          <.property prop_key="Interface Classes" key_class="text-gray-400 font-semibold">
            {@module.interface_classes |> Enum.join(", ")}
          </.property>

          <%= if @module.implementor do %>
            <.property prop_key="Implementor" key_class="text-gray-400 font-semibold">
              {@module.implementor}
            </.property>
          <% end %>

          <%= if @module.group do %>
            <.property prop_key="Group" key_class="text-gray-400 font-semibold">
              {@module.group}
            </.property>
          <% end %>

          <%= if @module.meaning do %>
            <.property prop_key="Meaning" key_class="text-gray-400 font-semibold">
              {inspect(@module.meaning)}
            </.property>
          <% end %>

          <%= for {property_name, property_value} <- @module.custom_properties do %>
            <.property prop_key={String.replace_prefix(property_name, "_", "")} key_class="text-gray-400 font-semibold">
              {inspect(property_value)}
            </.property>
          <% end %>
        </ul>
      ...
      <!-- Parameters -->
      <div class="border-4 border-zinc-500 bg-gray-800 rounded-lg p-4 mt-4">
        <h3 class="text-lg font-bold text-purple-400 mb-2">Parameters:</h3>
        <%= for {group_name, parameters} <- Enum.sort(@grouped_parameters) do %>
          ...
          <% else %>
            <div class="mt-4 border-l-4 border-purple-500 pl-4">
              <h4 class="text-xl font-semibold mb-2 text-purple-500 dark:text-purple-500">
      ...
      <%= if @module.commands != [] do %>
        <div class="border-4 border-zinc-500 bg-gray-800 rounded-lg p-4 mt-4">
          <h3 class="text-lg font-bold text-purple-400 mb-2">Commands:</h3>
  ...
  def parameter(assigns) do
    ...
    ~H"""
    <div class="mb-4 bg-gray-700 rounded-lg p-4 shadow-md">
      <div class="grid grid-cols-2 gap-4">
        <!-- Parameter Name -->
        <div>
          <span class="text-xl font-bold text-white">{Util.display_name(@parameter.name)}:</span>
          <ul class="mt-2 text-sm text-gray-300">
            ...
            <!-- Readonly -->
            <.property prop_key="Readonly" key_class="text-gray-400 font-semibold">
              <%= if @parameter.readonly do %>
                <span class="text-amber-400">🔒 Read-only</span>
              <% else %>
                <span class="text-green-400">✏️ Writable</span>
              <% end %>
            </.property>
            ...
            <%= if @parameter.visibility do %>
              <.property prop_key="Visibility" key_class="text-gray-400 font-semibold">
            ...
            <%= if @parameter.checkable do %>
              <.property prop_key="Checkable" key_class="text-gray-400 font-semibold">
                {@parameter.checkable}
              </.property>
            <% end %>
        ...
        <!-- Datainfo -->
        <div>
          <ul class="text-sm font-semibold text-gray-300">
            <.property prop_key="Datainfo" key_class="text-gray-400 font-semibold">
              <pre class="whitespace-pre-wrap break-words max-h-[60vh] overflow-y-auto dark:text-gray-200 font-mono text-xs bg-gray-800 p-2 rounded-lg">
                {@datainfo_pretty}
              </pre>
            </.property>
          </ul>
